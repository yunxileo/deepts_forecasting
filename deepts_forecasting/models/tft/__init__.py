...
class TemporalFusionTransformer(BaseModelWithCovariates):
    ...
    def get_attention_mask(
        self, encoder_lengths: torch.LongTensor, decoder_length: int
    ):
        """
    ...
    def interpret_output(
        ...
        attention = out["attention"][
            ...
            :,
            : out["encoder_lengths"].max() + attention_prediction_horizon,
        ].mean(1)
        ...
        if reduction != "none":  # if to average over batches
            ...
            for i in range(len(attention)):  # very inefficient but does the trick
                if (
                    ...
                    < attention.size(1) - attention_prediction_horizon - 1
                ):
            ...
            if reduction == "mean":
                attention = attention / encoder_length_histogram[1:].flip(0).cumsum(
                    0
                ).clamp(1)
