...
from torch.nn.modules import RNN as rnn
...
class TorchNormalizer(BaseEstimator, TransformerMixin):
    ...
    def __call__(self, data: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        if data["prediction"].ndim > norm.ndim:
            center = norm[:, 0].unsqueeze(-1).unsqueeze(-1)
            scale = norm[:, 1].unsqueeze(-1).unsqueeze(-1)
        else:
            center = norm[:, :, 0].unsqueeze(-1)
            scale = norm[:, :, 1].unsqueeze(-1)
        # transform
        y = data["prediction"] * scale + center
...
class GroupNormalizer(TorchNormalizer):
    ...
    def fit(self, y: pd.Series, X: pd.DataFrame):
        ...
        if len(self.groups) == 0:
            ...
            if self.method == "standard":
                self.norm_ = {
                    ...
                    .assign(
                        center=lambda x: x["center"], scale=lambda x: x.scale + self.eps
                    )
        ...
        if (
            (
                self.scale_by_group
                and any(
                    [(self.norm_[group]["scale"] < 1e-7).any() for group in self.groups]
                )
            )
            or (
                not self.scale_by_group
    ...
    def inverse_transform(
        self, y: pd.Series, X: pd.DataFrame = None, target_scale: torch.Tensor = None
    ):
        """
        ...
        return self(dict(prediction=y, target_scale=target_scale))
    ...
    def transform(
        ...
        return super().transform(
            y=y, return_norm=return_norm, target_scale=target_scale
        )
        ...
        for idx, normalizer in enumerate(self.normalizers):
            ...
            if isinstance(normalizer, GroupNormalizer):
                r = normalizer.transform(
                    y[idx], X=X, return_norm=return_norm, target_scale=scale
                )
            else:
                r = normalizer.transform(
